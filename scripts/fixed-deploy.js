...
async function main() {
  ...
  try {
    ...
    // 关键修复: 使用钱包地址作为USDC地址，而不是零地址
    // 在测试环境下，这只是一个占位符，因为我们不会实际使用USDC功能
    const usdcAddress = wallet.address; // 使用部署者的地址作为非零地址
    
    console.log('\n合约参数:');
    console.log(`- 标题: ${title}`);
    console.log(`- 选项: ${options.join(', ')}`);
    console.log(`- USDC地址: ${usdcAddress} (非零地址)`);
